...
var express = require('express')
  ...
  , tarefa = require('./routes/tarefa')
  , times = require('./routes/times')
  , campanhas = require('./routes/campanhas')
  , avaliacao = require('./routes/avaliacao')
 // , http = require('http')
  , path = require('path');
...
/* 
ftp
endereco: ag-br1-30.conteige.cloud
sebypu_scrumgame
porta: 21
senha: joAoG0m3z45
*/
...
//   url_sistema = "https://scrum3d.com/";
  //url_sistema = 'https://sebypu.conteige.cloud';
  url_sistema = 'https://scrumzando.com.br';

  pool = mysql.createPool({
    connectionLimit : 50,
    host: 'mysql-ag-br1-11.conteige.cloud',
    port: '3306', 
    user: 'sebypu_scrumgame',
    database: 'sebypu_scrumgame',
  ...
  console.log("Criou o pool de conexoes");
    ...
    pool = mysql.createPool({
        ...
        host: 'localhost',
        port: '3306', 
        user: 'stormUser',//este eh o nome do ususario que voce deve criar no mysqladmin
        // database: 'scrumbd',//banco do teste de 24/10/2020 em producao 
        // database: 'scrumbd-prod',//banco do teste de 24/10/2020 em producao 
        database: 'jogobd',//nome do banco de dados para criar no mysqladmin 
        password: '36912151821@@@'//senha do ususario do banco de dados 
      });  
}
...
app.post('/users/carregar_membros_time', user.carregar_membros_time);

app.post('/times/carregar_detalhe_time', times.carregar_detalhe_time);
app.post('/times/salvar_time', times.salvar_time);
app.get('/times/list', times.carregar_times);

app.get('/campanhas/list', campanhas.carregar_campanhas);
app.post('/campanhas/carregar_detalhe_campanha', campanhas.carregar_detalhe_campanha);
app.post('/campanhas/salvar_campanha', campanhas.salvar_campanha);

app.post('/avaliacao/salvar_avaliacao', avaliacao.salvar_avaliacao);
app.post('/avaliacao/carregar_ranking', avaliacao.carregar_ranking);
//Fazendo
app.get('/tarefa/list', tarefa.carregar_tarefas);
app.get('/tarefa/tarefas_quadro', tarefa.carregar_tarefas_quadro);
app.post('/tarefa/carregar_detalhe_tarefa', tarefa.carregar_detalhe_tarefa);
app.post('/tarefa/salvar_tarefa', tarefa.salvar_tarefa);

app.post('/quadro/atualizar_tarefa_quadro', tarefa.atualizar_etapa);
...
io.sockets.on('connection', function (socket) {
	...
	socket.on('subscribe', function(room) {
        console.log('Entrando no quadro do Time ', room);
        socket.join(room); 
    });
    ...
    socket.on('atualizar_quadros_pessoas_time', function (data) {

        console.log('atualizando quadros do time: ' + data.time);

        // vou atualizar a variavel 'data' com os dados de todas as colunas com base nos valores da tabela 'historias_jogo', que 
        // acabou de ser atualizado pela chamada da funcao 'saveHistory(numero, etapa, 0, 0)'

		socket.to(data.time).emit('aplicar_atualizacao_quadro', data); // envia mensagem para todos da room
	});
